...
export default function SimpleQuiz({ quiz, setChange, hubType, hubId }) {
    ...
    useEffect(() => {
      fetch(API_CATEGORIES)
        ...
        .then((categories) => categories.trivia_categories.filter((c) => c.id !== 13))
    ...
    return (
        <div className=" bg-gray-50 w-64 rounded-xl flex-col h-[450px] relative shadow-neutral-500 shadow-xl">
                ...
                <div className="p-4 text-black flex flex-col justify-center items-center">
                    ...
                    <div>
                        <p className=" text-sm bg-slate-100 text-slate-800 px-2 py-0.5 mt-3 rounded">{categoryName}</p>
                    </div>
                    <div className="flex text-sm gap-8 mt-4">
                        <div>
                            <span className="block text-slate-800 text-base font-semibold">{quiz.questions.length}</span>
                            <span className="block">{quiz.questions.length > 1 ? 'Questions' : 'Question'}</span>
                        </div>
                        <div>
                            <span className="block text-slate-800 text-base font-semibold">{quiz.timeLimit}m</span>
                ...
                {user && (userData.handle === quiz.creator || userData.role === 'admin') &&
                    <div className="absolute bottom-12 right-12 text-slate-400">
                        <SimpleQuizOptionsMenu quiz={quiz} id={quiz.id} handle={quiz.creator} setChange={setChange} hubType={hubType} hubId={hubId} />
                    </div>}
        </div>
    )
}
